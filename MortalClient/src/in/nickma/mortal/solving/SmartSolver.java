...
public class SmartSolver implements Solver {
    ...
    private boolean isSpaceEnd(final int x, final int y, final boolean[][] grid) {
        ...
        short neighbors = 0;
        for (Direction direction : Direction.values()) {
            int sideX = x + direction.getX();
            int sideY = y + direction.getY();
            if (isSpaceValid(sideX, sideY, grid)) {
                neighbors++;
            }
        }
        return neighbors == 1;
